      ...
      <main className={styles.main}>
        <div className={styles.container}>
          ...
          <p className={`${styles.description} fade-in`}>
            Demonstrating Redux Toolkit's createAsyncThunk with real API data
            from{" "}
            <a
              href="https://ahlesunnat.mws.ngo/api/test"
              target="_blank"
          ...
          <div className={styles.demoSection}>
            ...
            <div className={`${styles.demoCard} card-hover`}>
              <h2>Fetch All banners</h2>
              <p>Fetch all banners from the real API endpoint</p>

              <button
                onClick={handleFetchAlLbanners}
                disabled={loading}
                className={`btn-primary ${loading ? "loading" : ""}`}
              >
                {loading ? "Banners..." : "Fetch All Banners"}
              </button>

              {homeData && (
                <div className={styles.result}>
                  <h3>API Response Status: {homeData.status}</h3>
                  <p>
                    Total Banners: {homeData.data ? homeData.data.length : 0}
                  </p>
                </div>
              )}
            </div>
            ...
            {(error || createUserError) && (
              <div className={`${styles.demoCard} ${styles.errorCard}`}>
                ...
                <div className={styles.errorMessage}>
                  ...
                  {createUserError && (
                    <p>
                      <strong>Create User Error:</strong> {createUserError}
                    </p>
                  )}
                </div>
                <button
